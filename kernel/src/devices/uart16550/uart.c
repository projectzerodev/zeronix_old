...
void uart_init(void)
{
    ...
    outb(uart_base + UART_LCR, 0x03); // LCR=0x03: DLAB=0, bits 1-0=11 (8 bits), bits 6-5=00 (1 stop)
    // enable and reset TX/RX FIFOs (16-byte each), set trigger to 14 bytes
    outb(uart_base + UART_FCR, 0xC7); // FCR=0xC7: bits 7-6=11 (reset FIFOs), bits 2-0=111 (enable + 14-byte trigger)
    // configure modem control lines: Enable RTS/DTR, set OUT2 for potential IRQ routing
    outb(uart_base + UART_MCR, 0x0B); // MCR=0x0B: bits 3=1 (OUT2), 1=1 (RTS), 0=1 (DTR); no loopback
